...
public class ProducersServiceServlet extends HttpServlet {
    ...
    private static final String PAREMETR_CAR_ID= "car_id";
    private static final String PAREMETR_CAT_ID= "cat_id";
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (GET_ALL_METHOD.equalsIgnoreCase(queryMethod)) {
            ...
            String carIdString = request.getParameter(PAREMETR_CAR_ID);
            int carId = Integer.parseInt(carIdString);
            String catIdString = request.getParameter(PAREMETR_CAT_ID);
            int catId = Integer.parseInt(catIdString);
            Producer producer = new Producer(id, firstNameString,lastNameString,carId,catId);
            boolean created = facade.getProducerDao().add(producer);
            try (PrintWriter out = response.getWriter()) {
                if (created) {
            ...
            int catId = Integer.parseInt(catIdString);
            Producer producer = new Producer(id, firstNameString,lastNameString,carId,catId);
            boolean updated = facade.getProducerDao().update(producer);
            try (PrintWriter out = response.getWriter()) {
                if (updated) {
                    response.setStatus(HttpServletResponse.SC_OK);
            ...
            boolean deleted = facade.getProducerDao().delete(id);
            try (PrintWriter out = response.getWriter()) {
                if (deleted) {
                    response.setStatus(HttpServletResponse.SC_OK);
